...
impl CustomerService for CustomerServiceImpl {
    async fn register_customer(&self, request: Request<RegisterCustomerRequest>) -> Result<Response<CustomerResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let req = request.into_inner();

        // Check if exists
        if let Ok(Some(_)) = self.customer_repo.find_by_phone(&tenant_id, &req.phone).await {
            return Err(Status::already_exists("Customer with this phone already exists"));
        }
    ...
    async fn get_customer(&self, request: Request<GetCustomerRequest>) -> Result<Response<CustomerResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let id = Uuid::parse_str(&request.into_inner().id).map_err(|_| Status::invalid_argument("Invalid id"))?;

        let customer = self.customer_repo.find_by_id(&tenant_id, &id).await
            .map_err(|e| Status::internal(e.to_string()))?
    ...
    async fn find_customer_by_phone(&self, request: Request<FindCustomerByPhoneRequest>) -> Result<Response<CustomerResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let phone = request.into_inner().phone;

        let customer = self.customer_repo.find_by_phone(&tenant_id, &phone).await
            .map_err(|e| Status::internal(e.to_string()))?
            .ok_or_else(|| Status::not_found("Customer not found"))?;
    ...
    async fn list_customers(&self, request: Request<ListCustomersRequest>) -> Result<Response<ListCustomersResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let req = request.into_inner();
    ...
    async fn get_transaction_history(&self, request: Request<GetTransactionHistoryRequest>) -> Result<Response<TransactionHistoryResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let req = request.into_inner();
        let customer_id = Uuid::parse_str(&req.customer_id).map_err(|_| Status::invalid_argument("Invalid id"))?;
        
        let start_date = if let Some(ts) = req.start_date {
            chrono::DateTime::from_timestamp(ts.seconds, ts.nanos as u32).unwrap_or(Utc::now() - Duration::days(180))
        } else {
            Utc::now() - Duration::days(180) // 6 months default
        };

        let orders = self.order_repo.list_by_customer(&tenant_id, &customer_id, start_date).await
            .map_err(|e| Status::internal(e.to_string()))?;
        
        Ok(Response::new(TransactionHistoryResponse {
            transactions: orders.into_iter().map(|o| ProtoTransaction {
                id: o.id.to_string(),
