...
use infra::repository::{OrderRepository, InventoryRepository};
...
impl DeductionService {
    ...
    pub async fn deduct_stock_for_order(&self, tenant_id: &Uuid, branch_id: &Uuid, order_id: &Uuid) -> Result<()> {
        ...
        for (item, _) in items {
            ...
            let ingredients = self.recipe_repo.get_recipe_for_product(tenant_id, &item.product_id).await?;
