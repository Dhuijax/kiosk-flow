...
use proto_gen::report::{
    ...
    GetRevenueSummaryRequest, RevenueSummaryResponse,
    GetTopProductsRequest, TopProductsResponse, TopProductItem,
    GetSalesByPeriodRequest, SalesByPeriodResponse, SalesPeriodItem,
    PeriodType
};
...
impl ReportServiceImpl {
    ...
    fn parse_dates(&self, start: &str, end: &str) -> Result<(DateTime<Utc>, DateTime<Utc>), Status> {
...
impl ReportService for ReportServiceImpl {
    async fn get_revenue_summary(&self, request: Request<GetRevenueSummaryRequest>) -> Result<Response<RevenueSummaryResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let req = request.into_inner();

        let branch_id = if !req.branch_id.is_empty() {
            Some(Uuid::parse_str(&req.branch_id).map_err(|_| Status::invalid_argument("Invalid branch_id"))?)
        } else {
            None
        };
        ...
        let (total_revenue, total_orders) = self.report_repo.get_revenue_summary(&tenant_id, branch_id, start_dt, end_dt).await
    ...
    async fn get_top_products(&self, request: Request<GetTopProductsRequest>) -> Result<Response<TopProductsResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let req = request.into_inner();
        ...
        let rows = self.report_repo.get_top_products(&tenant_id, branch_id, start_dt, end_dt, limit).await
            .map_err(|e| Status::internal(e.to_string()))?;

        let items = rows.into_iter().map(|(pid, name, qty, rev)| {
            TopProductItem {
    ...
    async fn get_sales_by_period(&self, request: Request<GetSalesByPeriodRequest>) -> Result<Response<SalesByPeriodResponse>, Status> {
        let tenant_id = self.get_context(&request)?;
        let req = request.into_inner();
        ...
        let rows = self.report_repo.get_sales_by_period(&tenant_id, branch_id, start_dt, end_dt, period_str).await
            .map_err(|e| Status::internal(e.to_string()))?;

        let items = rows.into_iter().map(|(label, rev, count)| {
            SalesPeriodItem {
                period_label: label,
                revenue: self.map_money_to_proto(rev),
