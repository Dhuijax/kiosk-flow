      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-4">
          <div className="flex items-center gap-3 text-interaction animate-in fade-in slide-in-from-left duration-700">
            ...
            <span className="text-sm font-black uppercase tracking-[0.2em] italic">{t('infrastructure')}</span>
          </div>
          <h1 className="text-6xl font-black uppercase italic tracking-tighter leading-none">
            {t('storeBranch').split(' ')[0]} <span className="text-interaction">{t('storeBranch').split(' ').slice(1).join(' ')}</span>
          </h1>
                ...
                <div className="space-y-4 flex-1">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start gap-3 text-foreground/40">
                      ...
                      <p className="text-sm font-bold leading-relaxed">{branch.address || t('noAddress')}</p>
                    </div>
                    <div className="flex items-center gap-3 text-foreground/40">
                      <Phone className="w-5 h-5 shrink-0" />
                      <p className="text-sm font-bold tracking-wider">{branch.phone || t('noPhone')}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-8 border-t border-foreground/5 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <button 
                      ...
                      className="w-12 h-12 rounded-2xl border border-foreground/10 flex items-center justify-center hover:bg-interaction/10 hover:text-interaction transition-colors" title={t('edit')}
                    >
                    ...
                    {!branch.isMain && (
                      <button 
                        ...
                        className="w-12 h-12 rounded-2xl border border-foreground/10 flex items-center justify-center hover:bg-red-500/10 hover:text-red-500 transition-colors" title={t('delete')}
                      >
      ...
      {!loading && filteredBranches.length === 0 && (
        <div className="bg-surface border border-dashed border-foreground/20 rounded-[40px] py-32 flex flex-col items-center justify-center gap-8 text-center px-8">
          ...
          <div className="space-y-2">
            <h3 className="text-3xl font-black uppercase italic tracking-tighter">{t('noData')}</h3>
            <p className="text-foreground/40 font-medium">{t('searchPlaceholder')}</p>
          </div>
          <button 
